  ...
  try {
    ...
    if (!orderId || !placeId) {
      return NextResponse.json(
        { error: "orderId и placeId обязательны" },
        { status: 400 },
      )
    }

    const client = await getAuthenticatedRbxClient()
    const result = await client.orders.resendGamepass({ orderId, placeId })

    return NextResponse.json({ success: true, result })
  } catch (error) {
    console.error("POST /api/admin/rbx/orders/resend error:", error)
